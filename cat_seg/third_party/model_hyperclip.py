...
from .hyperclip import set_oft, set_oft_vision
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, layer: int, attn_mask: torch.Tensor = None, 
                 attn_layer_R = None, attn_relation_l_R = None, attn_relation_m_R = None):
        super().__init__()
        self.n_head = n_head
        ...
        if attn_layer_R is not None:
            #if layer < 11:
            self.attn_q_proj_oft_layer_R = attn_layer_R
        if attn_relation_l_R is not None:
            #if layer < 11:
            self.attn_q_proj_oft_relation_l_R = attn_relation_l_R
        if attn_relation_m_R is not None:
            #if layer < 11:
            self.attn_q_proj_oft_relation_m_R = attn_relation_m_R
...
class Transformer(nn.Module):
    def __init__(self,  attn_layer_R: nn.parameter, attn_relation_l_R: nn.parameter, attn_relation_m_R: nn.parameter, 
                 width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        super().__init__()
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, layer, attn_mask, 
                                                                attn_layer_R, attn_relation_l_R, attn_relation_m_R) for layer in range(layers)])
...
class VisualTransformer(nn.Module):
    def __init__(self, attn_layer_R: nn.parameter, attn_relation_l_R: nn.parameter, attn_relation_m_R: nn.parameter, 
        ...
        self.transformer = Transformer(attn_layer_R, attn_relation_l_R, attn_relation_m_R, width, layers, heads)
...
class LinearFFN(nn.Module):
    ...
    def __init__(self, dim: int, hidden_dim: int = None, bias: bool = False):
        ...
        if hidden_dim is None:
            # 传统 Transformer FFN 的常见默认值
            hidden_dim = 4 * dim

        # 上投影层：将维度从 dim 扩展到 hidden_dim
        self.w1 = nn.Linear(dim, hidden_dim, bias=bias)
        
        # 下投影层：将维度从 hidden_dim 缩减回 dim
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        执行 x -> w1(x) -> w2(w1(x)) 的线性变换。
        """
        # 通过上投影层
        hidden_state = self.w1(x)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        num = 10
        attn_layer_R_t = torch.eye(dimension, dtype=torch.float16)
        attn_layer_R_t = attn_layer_R_t.unsqueeze(2).unsqueeze(3)
        attn_layer_R_t = attn_layer_R_t.expand(dimension, dimension, 4 * transformer_layers, 4)

        attn_layer_R_v = torch.eye(dimension, dtype=torch.float16)
        attn_layer_R_v = attn_layer_R_v.unsqueeze(2).unsqueeze(3)
        attn_layer_R_v = attn_layer_R_v.expand(dimension, dimension, 4 * transformer_layers, 6)

        attn_layer_R = torch.cat([attn_layer_R_t, attn_layer_R_v], dim=-1)
        attn_layer_R = nn.Parameter(attn_layer_R, requires_grad=True)
        net_l_R = LinearFFN(dim=4*transformer_layers)
        net_m_R = nn.ModuleList([LinearFFN(dim=num) for _ in range(12)])
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisualTransformer(
                attn_layer_R = attn_layer_R,
                attn_relation_l_R = net_l_R,
                attn_relation_m_R = net_m_R,
                input_resolution=image_resolution,
        ...
        set_oft_vision(self.visual, dim=128, hidden_size=vision_width, length=transformer_layers-prompt_length)
        set_oft(self.transformer, dim=128, hidden_size=transformer_width, length=transformer_layers-prompt_length)
...
def init_identity(m):
    if isinstance(m, nn.Linear):
        if m.weight.size(0) == m.weight.size(1):  # 检查是否为方阵
            torch.nn.init.eye_(m.weight)  # 初始化为单位矩阵
        else:
            torch.nn.init.kaiming_normal_(m.weight)  # 或者用其他适当的初始化
